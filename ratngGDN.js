(function() {
    ...
    Lampa.Lang.add({
        ratimg_omdb_avg: {
            ru: 'ИТОГ', en: 'TOTAL', uk: 'ПІДСУМОК', be: 'ВЫНІК', pt: 'TOTAL', zh: '总评', he: 'סה"כ', cs: 'VÝSLEDEK', bg: 'РЕЗУЛТАТ'
        },
        loading_dots: {
            ru: 'Загрузка...', en: 'Loading...', uk: 'Завантаження...', be: 'Загрузка...', pt: 'Carregando...', zh: '加载中...', he: 'טוען...', cs: 'Načítání...', bg: 'Зареждане...'
        },
        maxsm_omdb_oscars: {
            ru: 'Оскары', en: 'Oscars', uk: 'Оскари', be: 'Оскары', pt: 'Oscars', zh: '奥斯卡奖', he: 'אוסקר', cs: 'Oscary', bg: 'Оскари'
        },
        source_imdb: {
            ru: 'IMDB', en: 'IMDB', uk: 'IMDB', be: 'IMDB', pt: 'IMDB', zh: 'IMDB', he: 'IMDB', cs: 'IMDB', bg: 'IMDB'
        },
        source_tmdb: {
            ru: 'TMDB', en: 'TMDB', uk: 'TMDB', be: 'TMDB', pt: 'TMDB', zh: 'TMDB', he: 'TMDB', cs: 'TMDB', bg: 'TMDB'
        },
        source_rt: {
            ru: 'Rotten Tomatoes', en: 'Rotten Tomatoes', uk: 'Rotten Tomatoes', be: 'Rotten Tomatoes', pt: 'Rotten Tomatoes', zh: '烂番茄', he: 'Rotten Tomatoes', cs: 'Rotten Tomatoes', bg: 'Rotten Tomatoes'
        },
        source_mc: {
            ru: 'Metacritic', en: 'Metacritic', uk: 'Metacritic', be: 'Metacritic', pt: 'Metacritic', zh: 'Metacritic', he: 'Metacritic', cs: 'Metacritic', bg: 'Metacritic'
        }
    });
    ...
    var css = 
        ...
        // Стилі для іконок
        ".source-icon { height: 14px; width: auto; display: inline-block; vertical-align: middle; }" +
        ".source-icon.imdb { height: 15px; }" +
        ".source-icon.oscar { fill: #ffcc00; }" +
        ".source-icon.tmdb { height: 14px; }" +
        ".source-icon.rt { height: 14px; }" +
        ".source-icon.mc { height: 14px; }";
    ...
    // Головна функція для правильної вставки
    function insertAfterRating(rateLine, element) {
        // Шукаємо останній рейтинг (IMDB або TMDB)
        var lastRating = rateLine.find('.rate--imdb, .rate--tmdb').last();
        
        if (lastRating.length) {
            lastRating.after(element); // Вставляємо ПІСЛЯ них
        } else {
            // Якщо немає ні того, ні іншого, шукаємо Оскари або вставляємо на початок, але перед бейджами
            var oscars = rateLine.find('.rate--oscars');
            if(oscars.length) {
                oscars.after(element);
            } else {
                // Якщо є інші елементи (бейджи), вставляємо перед ними, якщо це перший рейтинг
    ...
    function updateUI(render, rateLine, data, card) {
        ...
        // Створюємо елементи
        if (data.rt) fragment.appendChild(createRateItem(data.rt.toFixed(1), 'source_rt', 'rate--rt', ICONS.rt));
        if (data.mc) fragment.appendChild(createRateItem(data.mc.toFixed(1), 'source_mc', 'rate--mc', ICONS.mc));
        ...
        if (data.oscars) {
            var oscarsEl = createRateItem(data.oscars, 'maxsm_omdb_oscars', 'rate--oscars', ICONS.oscar);
        ...
        if (imdbRate.length) {
            ...
            imdbRate.children().eq(1).html(ICONS.imdb + Lampa.Lang.translate('source_imdb'));
        }

        // Оновлюємо TMDB
        var tmdbRate = render.find('.rate--tmdb');
        if (tmdbRate.length) {
            tmdbRate.children().eq(1).html(ICONS.tmdb + Lampa.Lang.translate('source_tmdb'));
        }

        calcAverage(rateLine, card, data);
    }

    function createRateItem(value, labelKey, className, iconSvg) {
        var div = document.createElement('div');
        div.className = 'full-start__rate ' + className;
        var labelText = Lampa.Lang.translate(labelKey);
        div.innerHTML = '<div>' + value + '</div><div class="source--name">' + (iconSvg || '') + labelText + '</div>';
        return div;
    }
